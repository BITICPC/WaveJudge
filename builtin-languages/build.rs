...
fn copy_scripts<P>(out_dir: &P)
    where P: ?Sized + AsRef<Path> {
    let out_dir = out_dir.as_ref().to_owned();

    // Copy all script files under script/ to the output directory.
    let script_dir = PathBuf::from("./scripts");
    let read_script_dir = script_dir.read_dir()
        .expect("Failed to read content of script directory.");

    for script_file in read_script_dir {
        let script_file = script_file.expect("failed to iterate content of script directory.");
        let script_file = script_file.path();

        let script_file_name = script_file.file_name().unwrap();
        let mut target_script_file = out_dir.clone();
        target_script_file.push(script_file_name);

        eprintln!("Copying script file \"{}\" to output directory", script_file.display());
        std::fs::copy(script_file, target_script_file)
            .expect("Failed to copy script file.");
    }
}
